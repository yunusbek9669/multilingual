            ...
            <div class="ml-table-responsive">
                <table class="ml-table">
                    ...
                    <tr>
                        ...
                        <?php if (!empty($translates)): ?>
                            <?php foreach ($translates['header']['language'] as $key => $not_translated): ?>
                                <th><?php echo $key . ($default_language['name'] === $key ? '' : ' <span class="ml-not-translated ' . ($not_translated > 0 ? 'has' : 'not') . '">' . $not_translated . '</span>') ?></th>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </tr>
                    </thead>
                    <?php foreach ($translates['body'] as $key => $row): $not_tran_count = 0; ?>
                        ...
                        <tr>
                            <td rowspan="<?php echo 1 + count($row['translate']) ?>"
                                style="width: 50px; text-align: center"><a
                                        href="<?php echo Url::to(['translate', 'table_name' => $row['table_name'], 'table_iteration' => $row['table_iteration'], 'attributes' => array_keys($row['translate'])]) ?>"
                                        class="btn btn-<?php echo $row['is_full'] ? 'info' : 'warning' ?>"
                                        data-row="<?php echo count($translates['header']['language']) * count($row['translate']) ?>">
                            ...
                            <td rowspan="<?php echo 1 + count($row['translate']) ?>"
                                style="color: #979aa6; font-style: italic; font-weight: bold;">
                        ...
                        <?php endforeach; ?>
                        <tr>
                            <td colspan="<?php echo 3 + count($translates['header']['language']) ?>"
                                style="padding: 0"></td>
                        </tr>
                        </tbody>
                    <?php endforeach; ?>
